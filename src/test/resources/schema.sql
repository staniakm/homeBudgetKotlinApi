...
'
    ...
    begin
        if exists(select 1
            ...
            begin
                insert into assortment(name)
                values (UPPER(product));
                select id
                into asoId
                from assortment
                where name = UPPER(product);
        ...
        if not exists(select 1
            ...
            begin
                ...
                select invoice_id,
                       ...
                       l.quantity,
                       l.price,
                       concat(''Rachunki '', extract(month from current_date)::varchar),
                       false,
                       a.category
...
'
    begin
        ...
        if exists(select id
                  from account
                  where id = new_account_id) then
            update account
            set money = money + invoice.sum
            from invoice
...
CREATE FUNCTION public.generatereportbycategory()
    RETURNS TABLE
            (
...
CREATE FUNCTION public.generatereportbycategoryandaccount()
    RETURNS TABLE
            (
    ...
    begin
        return query
            ...
            SELECT t.owner_name,
                   t.category_name,
                   t.sum,
                   cast((t.sum / (select sum(t1.sum)
                                  from t as t1)) * 100 as decimal(10, 2)) as percentage
            from t
...
CREATE FUNCTION public.generatestandardreport()
    RETURNS TABLE
            (
                month       integer,
                year        integer,
            ...
            SELECT t.month::INTEGER,
                   ...
                   coalesce((select coalesce(sum(sums), 0)
                             ...
                             where k.owner = 1
                               ...
                               and t1.year = t.year), 0)                                              as owner_1,
                   coalesce((select coalesce(sum(sums), 0)
                             from t t2
                             ...
                             where k.owner = 2
                               ...
                               and t2.year = t.year), 0)                                              as owner_2,
                   coalesce((select coalesce(sum(sums), 0)
                             from t t3
                                      join account k on k.ID = t3.account
                             where k.owner = 3
                               and k.del = false
                               and t3.month = t.month
                               and t3.year = t.year), 0)                                                 common,
                   ...
                   cast(coalesce((select sum(value)
                                  from income inc
                                  where extract(year from inc.date) = t.year
                                    and extract(month from inc.date) = t.month), 0) -
                        cast(sum(sums) as decimal(9, 2)) as decimal(9, 2))                            as savings
            from t
...
'
    begin
        ...
        where b1.id = b.id
          ...
          and b1.year = extract(year from recalculation_date);

        update budget
        set percentage = (used * 100 / planned)
        where month = extract(month from recalculation_date)
          and year = extract(year from recalculation_date)
          and planned > 0;
...
'
    ...
    begin
        ...
        begin
            ...
            set price    = (i.amount * i.unit_price) - i.discount,
                category = a.category
            from assortment a
            where i.assortment = a.id
              and i.invoice = invoice_to_update
              and i.del = false;
            select date
            into month_date
            from invoice
...
'
    begin
        if (select planned
            from budget
            where id = selectedBudget) > 0 then
            update budget
            set percentage = (used * 100 / planned)
...
CREATE FUNCTION public.showinvoicelist()
    RETURNS TABLE
            (
                invoice_id     bigint,
                invoice_date   text,
...
CREATE PROCEDURE public.updateasocategory(IN assortment_id integer, IN category_name character varying,
                                          IN assortment_name character varying,
                                          IN category_id integer DEFAULT '-1'::integer)
AS
'
    begin
        if (category_id = -1 and category_name <> '''') then
            ...
            begin
                ...
                where id = assortment_id
                  and coalesce(category, -1) <> category_id;
            end;
        end if;
...
ALTER TABLE public.account_owner_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.assortment_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.automatic_invoice_products_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.automatic_invoice_products
    OWNER TO postgres;
...
ALTER TABLE public.budget_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.counter_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.income_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.invoice_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.invoice_details_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.media_type_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.media_usage_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.salary_type_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.salary_type
    OWNER TO postgres;
...
ALTER TABLE public.shop_id_seq
    OWNER TO postgres;
...
ALTER TABLE public.shop_assortment
    OWNER TO postgres;
...
ALTER TABLE ONLY public.media_type
    ALTER COLUMN id SET DEFAULT nextval('public.media_type_id_seq'::regclass);
...
ALTER TABLE ONLY public.media_usage
    ALTER COLUMN id SET DEFAULT nextval('public.media_usage_id_seq'::regclass);
