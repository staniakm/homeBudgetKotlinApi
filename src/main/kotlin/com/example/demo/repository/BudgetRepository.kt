...
class BudgetRepository(private val helper: RepositoryHelper,
    ...
    fun updateBudget(date: LocalDate, updateBudget: UpdateBudgetDto) {
        client.sql(queryProvider.UPDATE_MONTH_BUDGE_DETAILS)
...
//                .bind("month", date.monthValue)
    ...
    private fun getBudgetItem(date: LocalDate): Mono<BudgetItem> {
        return helper.findOne(queryProvider.GET_MONTH_BUDGE_DETAILS, budgetItemMapper) {
            bind("year", date.year)
                    .bind("month", date.monthValue)
        }
    }
    ...
    private fun getMonthBudgetForCategory(date: LocalDate, category: String): Flux<MonthBudget> {
        return helper.getList(queryProvider.GET_MONTH_BUDGET_FOR_CATEGORY, monthBudgetMapper) {
            bind("year", date.year)
                    .bind("month", date.monthValue)
    ...
    private fun getMonthBudgets(date: LocalDate): Flux<MonthBudget> {
        return helper.getList(queryProvider.GET_MONTH_BUDGET, monthBudgetMapper) {
            bind("year", date.year)
                    .bind("month", date.monthValue)
        }
    }
